...
async def get_scheduled_activities(db: DB):
    """Get all scheduled_activities"""
    scheduled_activities = service.get_scheduled_activities(db)
...
async def get_scheduled_activity_by_id(db: DB, scheduled_activity_id: str):
    """Get scheduled_activity by ID"""
    scheduled_activity = service.get_scheduled_activity_by_id(db, scheduled_activity_id)
...
@router.post("/",
             ...
             response_model=dto_out.GetScheduledActivity)
async def create_scheduled_activity(db: DB, request: dto_in.CreateScheduledActivity):
    """Create a new scheduled_activity"""
    scheduled_activity = service.create_scheduled_activity(db, request)
    return dto_out.GetScheduledActivity(scheduled_activity=scheduled_activity)


@router.delete("/{scheduled_activity_id}",
               status_code=status.HTTP_204_NO_CONTENT)
async def delete_scheduled_activity(db: DB, scheduled_activity_id: str):
    """Delete a scheduled_activity by ID"""
    service.delete_scheduled_activity(db, scheduled_activity_id)


@router.put("/{scheduled_activity_id}",
            status_code=status.HTTP_200_OK,
            response_model=dto_out.GetScheduledActivity)
async def update_scheduled_activity(db: DB, scheduled_activity_id: str, request: dto_in.UpdateScheduledActivity):
    """Update a scheduled_activity by ID"""
    scheduled_activity = service.update_scheduled_activity(db, scheduled_activity_id, request)
    return dto_out.GetScheduledActivity(scheduled_activity=scheduled_activity)
...
async def insert_scheduled_activities_bulk(
    ...
    service.insert_scheduled_activities_bulk(db, request)
